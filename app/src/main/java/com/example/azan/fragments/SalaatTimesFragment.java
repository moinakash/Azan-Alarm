...
import com.example.azan.Constants;
import com.example.azan.R;
import com.example.azan.SetAlarmActivity;
import com.example.azan.scheduler.RamadanAlarmReceiver;
import com.example.azan.scheduler.SalaatAlarmReceiver;
import com.example.azan.util.AppSettings;
import com.example.azan.util.PrayTime;
...
public class SalaatTimesFragment extends Fragment implements Constants {
  ...
  protected void init(View view) {
    ...
    LinkedHashMap<String, String> prayerTimes =
        PrayTime.getPrayerTimes(getActivity(), mIndex, lat, longi);
    ...
    if (!sIsAlarmInit) {
      if (AppSettings.getInstance().isDefaultSet()) {
        AppSettings.getInstance().setLatFor(mIndex, lat);
        AppSettings.getInstance().setLngFor(mIndex, longi);
        updateAlarmStatus();
        sIsAlarmInit = true;
      }
    }
  ...
  private void setAlarmButtonClickListener(TextView alarm, int index) {
    alarm.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        settings.setLatFor(mIndex, lat);
        settings.setLngFor(mIndex, longi);
